 ...
 * проверка на отключенные модули вне их самих, отключение нек. модулей
 * в папке modules(напр. messages, statics, chat, news, search_module и class.search в т.ч.)
 * и части модулей(напр. invites, bookmarks(в т.ч. и кнопки) в usercp)
 * @todo снабдить документацией функции JS в папке js.
 * @todo необяз.: кеш шаблонов: не кешируется if и assign
 * (при первой компиляции assign выполняется, но остаётся в шаблоне), 
 * после foreach пишем в кеш assign со значениями ключа и значения, 
 * так же в кеш пишем значения, которые добавляются с начала слежения за кешем.
 * всё остальное успешно компилируется.
 */
