...
(function($) {
        ...
        setup : function() {
            if (this.addEventListener)
                this.addEventListener('DOMMouseScroll',
                    $.event.special.mousewheel.handler, false);
    ...
    $.fn.paginator = function(s) {
        ...
        // Ой, какой костыль, какой костыль. Но мне ЛЕНЬ переписывать, посему пущай лучше будет так.
        // Да и, наверное, не перепишу, как надо, ибо не хватит знаний(мозгов?) :)
        reset_paginators[reset_paginators.length] = options.clickHandler;
        $.extend(options, s);
        ...
        : options.pageCurrent;
        if (!options.baseurl.match(/%number%/i))
            options.baseurl += '%number%';
        ...
        function getLink(page) {
            return options.baseurl.replace(/%number%/i, options
                .builCounter(page));
        }
        ...
        function makePagesTableHtml() {
            ...
            : options.pagesTotal;
            var next = '<a href="' + getLink(next_page) + '" rel="' + next_page
            + '">%next%</a>';

            var last = '<a href="' + getLink(options.pagesTotal) + '" rel="'
            + options.pagesTotal + '">%last%</a>';

            var prior_page = (parseInt(options.pageCurrent) > 1) ? parseInt(options.pageCurrent) - 1
            : 1;
            var prior = '<a href="' + getLink(prior_page) + '" rel="'
            + prior_page + '">%prior%</a>';

            var first = '<a href="' + getLink(1) + '" rel="' + 1
            + '">%first%</a>';
            ...
            code = '<table width="100%">' + '<tr>' + '<td class="left top">'
            + top_left + "<div class='bottom'>" + bottom_left
            + '</div></td>' + '<td rowspan="2" align="center">'
            + '<table class="pages">' + '<tr>';
            for (i = 1; i <= options.pagesSpan; i++) {
                code += '<td width="' + tdWidth + '" />';
            }
            code += '</tr>' + '<tr>' + '<td colspan="' + options.pagesSpan
            + '">' + '<div class="scroll_bar">'
            + '<div class="scroll_trough" />'
            + '<div class="scroll_thumb">'
            + '<div class="scroll_knob" />' + '</div>'
            + '<div class="current_page_mark" />' + '</div>' + '</td>'
            + '</tr>' + '</table>' + '</td>' + '<td class="right top">'
            + top_right + "<div class='bottom'>" + bottom_right
            + '</div></td>' + '</tr>' + '</table>';
        ...
        function setPageCurrentPointWidth() {
            $(html.pageCurrentMark).css("width",
                html.pageCurrentMark.widthPercent + '%');
        ...
        function initEvents() {
                ...
                if (e.ctrlKey) {
                    switch (e.keyCode ? e.keyCode : e.which ? e.which : null) {
                        case 0x25: // previous page
                            executeLink($(options.returnOrder ? '.right.top a'
                                : '.left.top a', html.holder));
                            break;

                        case 0x27: // next page
                            executeLink($(options.returnOrder ? '.left.top a'
                                : '.right.top a', html.holder));
                            break;
                    }
                }
            })
            ...
            .bind(
                'mousewheel',
                function(e) {
                    selector = (e.delta > 0) ? (options.returnOrder ? '.right.top a'
                        : '.left.top a')
                    : (options.returnOrder ? '.left.top a'
                        : '.right.top a');
                    cellFirstValue = options.pagesSpan;
                    html.scrollThumb.xPos += (e.delta > 0 ? -1 : 1)
                    * (($(html.table).width() - $(
                        html.scrollThumb).width()) / options.pagesTotal/*
																								 * *
																								 * options.pagesSpan
																								 */);
                    moveScrollThumb();
        ...
        function bindLink(el) {
            if ($.isFunction(options.clickHandler)) {
                $(el).find('a').bind(
                    'click',
                    function() {
                        var what = $(this).attr('rel')?$(this).attr('rel'):$(this).text();
                        return options.clickHandler(options.builCounter(what));
                    });
            }
        }
        ...
        function drawPages() {
            ...
            for ( var i = 0; i < html.tdsPages.length; i++) {
                ...
                if (cellCurrentValue == options.pageCurrent) {
                    ...
                    data = '<span><a href="' + getLink(cellCurrentValue) + '">'
                    + cellCurrentValue + '</a></span>';
                }
                ;
