...
(function($) {
    ...
    .extend( {
        tablesorter : new function() {
            ...
            function buildHeaders(table) {
                ...
                $remote = ($("thead th", table).is(".js_remote") ? true
                    : false);
                $tableHeaders
                .each(function(index) {
                    if ($(this).is(".js_nosort"))
                        return;
                    this.count = 0;
                    this.column = index;
                    ...
                    .click(
                        function(e) {
                            ...
                            if (!this.sortDisabled
                                ...
                                // false
                                if ($
                                    .isFunction(config.remote_function))
                                    config
                                    .remote_function(config.sortList);
                                return false;
                            }
                        // cancel selection
                        })
    ...
    ts.addParser( {
        id : "size",
        is : function(s) {
            var reg = /^([0-9]+).([0-9]+)\s*&nbsp;*(k|m|g|t)Bytes$/i;
            return reg.test(s);
        },
        format : function(s) {
            var reg = /^([0-9]+).([0-9]+)\s*&nbsp;*(k|m|g|t)Bytes$/i;
            var arr = s.match(new RegExp(reg));
            if (typeof (arr) == 'object' && (arr instanceof Array)) {
                if (arr[3] == "t")
                    ...
                    bytes = 1024;
                return $.tablesorter.formatFloat((arr[1] + arr[2] * 0.1)
                    * bytes);
            } else
                return null;
        },
