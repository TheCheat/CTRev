...
// options --------------------------------------------------------------------
	...
	, codeLanguage:       function( el ) {
		...
		return recipeName ? recipeName : 'text';
	}
...
//					window.clipboardData.setData( 'Text', selected ); //I couldn't find anything similar for Mozilla
	...
	function addLineNumbers( el ) {
		...
		function makeListItem1( not_last_line, not_last, last, open, lln ) {
			var close = open ? '</span>' : '';
			var aux = '';
			var subclass = (lln % 2 == 0?' class="second_line"':'');
			if( not_last_line ) {
				aux = '<li'+subclass+'>' + open + not_last + close + '</li>';
			}
			else if( last ) {
				aux = '<li'+subclass+'>' + open + last + close + '</li>';
			}
			return aux;
		} // makeListItem1

		function makeListItem2( not_last_line, not_last, last, prev_li, lln ) {
			var aux = '';
			if( prev_li ) {
				aux = prev_li;
			}
			else {
				aux = makeListItem1( not_last_line, not_last, last, '', lln )
			}
			return aux;
		} // makeListItem2
		...
		var list_items = html
					...
					if (/<br>/i.test(content)) {
						...
						var aux = (before ? before + br : '') //+ replace1( lastPiece + content, open );
							...
							function( tmp, not_last_line, not_last, last ) {
								last_list_num++;
								var aux2 = makeListItem1( not_last_line, not_last, last, open, last_list_num );
								return aux2;
							} 
						);
						return aux;
			...
			.replace( /(<li>.*?<\/li>)|((.*?)(?:&nbsp;|\xA0)<br>)|(.+)/ig, 
				...
				last_list_num++;
					var aux2 = makeListItem2( not_last_line, not_last, last, prev_li, last_list_num );
					return aux2;
				} 
			)
