...
(function($) {
	...
	$.jCal = function (target, opt) {
		opt = $.extend({
			...
			changeMY:		function (days) {return true; },
	...
	function drawCalControl (target, opt) {
						...
						$('.jCalMo .monthSelect', e.data._target).bind('mouseover mouseout click', $.extend( {}, e.data ), 
							function (e) {
								if ( e.type == 'click' ) {
									...
									if ( typeof opt.changeMY == 'function' ) opt.changeMY(new Date($(this).attr('id').replace(/_/g, '/')), e.data);
								}
								else
		...
		$(target).find('.jCal .left').bind('click', $.extend( {}, opt ),
			function (e) {
				...
				e.data.day = new Date( $('div[id*=' + e.data.cID + 'd_]', e.data._target).first().attr('id').replace(e.data.cID + 'd_', '').replace(/_/g, '/') );
				...
				$('.jCalMove', e.data._target).css('margin-left', ( ( mD.w / e.data.showMonths ) * -1 ) + 'px').css('opacity', 0.5).animate({ marginLeft:'0px' }, e.data.scrollSpeed,
					function () {
					if ( typeof opt.changeMY == 'function' ) {date = new Date(opt.day);date.setMonth(opt.day.getMonth()-1);opt.changeMY(date,e.data);}
	...
	function drawCal (target, opt) {
		...
		for ( var d = offsetDayStart, dE = ( copt.fd + copt.ld + ( 7 - offsetDayEnd ) ); d < dE; d++) {
			$dcheck = ( opt.dCheck( new Date( (new Date( fd.getTime() )).setDate( d - ( copt.fd - opt.dayOffset ) ) ) ) || 'invday' );
			if (typeof $dcheck == "object") {
				$before = $dcheck[0];
				$after = $dcheck[2];
				$dcheck = $dcheck[1];
			} else {
			...
			$(target).append(
				(( d <= ( copt.fd - opt.dayOffset ) ) ?
					...
					: ( ( d > ( ( copt.fd - opt.dayOffset ) + copt.ld ) ) ?
						...
						: $before +  '<div id="' + opt.cID + 'd_' + (fd.getMonth() + 1) + '_' + ( d - ( copt.fd - opt.dayOffset ) ) + '_' + fd.getFullYear() + '" class="' +
							$dcheck +
							'">' + ( d - ( copt.fd - opt.dayOffset ) )  + '</div>' + $after
					) 
				)
			);
		}
